...
const FOG_CELL_SIZE: float       = 4.0

# ── 颜色 ─────────────────────────────────────────────────────────────────
const COLOR_FLOOR_LARGE := Color(0.20, 0.14, 0.10, 0.95)  # 大厅/宽廊（深棕）
const COLOR_FLOOR_SMALL := Color(0.30, 0.21, 0.14, 0.95)  # 窄走廊（浅棕）
const COLOR_FLOOR_EDGE  := Color(0.50, 0.36, 0.20, 0.35)  # 矩形描边
...
func _do_draw(canvas: Control) -> void:
	...
	for i in range(_walk_rects.size()):
		...
		# 将矩形细分为 FOG_CELL_SIZE 大小的格子，只画在探索圆内的格子
		var cx: float = r[0]
		while cx < r[1]:
			...
			while cz < r[3]:
				...
				if _in_fog(cell_cx, cell_cz):
					...
					canvas.draw_rect(Rect2(tl, br - tl), floor_color)

				cz = cz_end
			cx = cx_end

		# 矩形整体描边（只要该矩形有任意格子被探索就画描边）
		# 检查矩形中心是否被探索（快速近似）
		var mid_x: float = (float(r[0]) + float(r[1])) * 0.5
		var mid_z: float = (float(r[2]) + float(r[3])) * 0.5
		if _in_fog(mid_x, mid_z):
			var tl_raw := _world_to_ui(r[0], r[2], ms)
			var br_raw := _world_to_ui(r[1], r[3], ms)
			canvas.draw_rect(Rect2(tl_raw, br_raw - tl_raw), COLOR_FLOOR_EDGE, false, 1.0)
