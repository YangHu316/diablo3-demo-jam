...
func _apply_element_tint(elem: String) -> void:
	...
	# V3.13:冰箭蓝色拖尾 — 直接用 BinbunVFX vfx_02(蓝色魔法粒子)文件,
	# 替代 V3.8 的程序化 CPU 雪花。视觉跟 E 技能 vfx_04 橙红对位形成色彩区分。
	if elem == "frost":
		_spawn_frost_trail_binbun()

# V3.13:冰箭蓝色拖尾 — 用 BinbunVFX vfx_02 替代旧 CPU 雪花,跟 E 的 vfx_04 橙红对位
...
func _spawn_frost_trail_binbun() -> void:
	...
	if trail is Node3D:
		# Godot 4 Transform3D 没有 12-float 构造器,用 4×Vector3(X 轴 / Y 轴 / Z 轴 / origin)
		(trail as Node3D).transform = Transform3D(Vector3(0, 0, -0.3), Vector3(0, 0.3, 0), Vector3(0.3, 0, 0), Vector3.ZERO)
